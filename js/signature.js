...
$.signature = function (callback) {
    ...
    var Title = ('<div class="" style="height: 36px;background-color: #fff;padding: 15px 15px 10px;font-size: 18px;text-align:left">请签名</div>');
    canvasBody = $('<div class="signature-body"></div>');
    var footer = $('<div class="signature-footer"></div>');
    footer.append('<input type="button" class="signature-btn-cancel"  value="返回" />');
    footer.append('<input type="button" class="signature-btn-ok"  value="确定" />');
    footer.append('<input type="button" class="signature-btn-clear"  value="清除" />');
    footer.append('<input type="hidden" id="signatureScrollTop" />');

    canvasDiv = $('<div class="signature-div signature-fullscreen"></div>');
    canvasDiv.append(Title);
...
function canvasDraw(target, options, fgcanvas, fgctx) {
    ...
    function BeginPolyline(wrapper, id, x, y) {
        ...
        foregroundPolylines[id.toString()] = {
            points: [{
                x: x,
                y: y
            }]
    ...
    function ExtendPolylineTo(wrapper, id, x, y) {
        if (foregroundPolylines[id.toString()])
            foregroundPolylines[id.toString()].points.push({
                x: x,
                y: y
            });
        RequestRedraw();
